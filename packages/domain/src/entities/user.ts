...
/**
 * ULID id prefix for users: `usr_01HW…`. A User's id is the same {@link UserId}
 * an envelope's `updatedBy` references — the acting user in the conflict UI is a
 * row in this table — so the brand is shared, not redefined.
 */
 ...
 * 'secretary'` (other roles from {@link Role} exist but are unused in the MVP).
 * One `users` table is the single credential store, resolved by `username` at
 * login.
 *
 * SYNCED, sync-safe entity: carries the full envelope so accounts converge
 * across a center's laptops (soft-delete only; a removed employee is a
 * tombstone, never a hard delete). `passwordHash` and `setupCodeHash` are opaque
 * Argon2id PHC strings produced by the {@link PasswordHasher} in the main
 * process — the domain never inspects them and no plaintext (password or setup
 * code) is ever stored.
 *
 * Uniqueness is EXACT, not fuzzy: `username` is unique per center among live
 * rows (a partial unique index on the normalized form, like `Subject.code`).
 * Users are created deliberately by the director, not matched-in from two
 * devices, so they carry no people-like `naturalKey` — there is nothing to
 * fuzzy-dedupe; a same-username clash is a hard rejection, not a merge.
 *
 * A user is created in one of two shapes:
 * - **owner (first-run):** `passwordHash` set immediately, no setup code.
 * - **employee (invited):** `passwordHash` is `null` until the one-time setup
 *   code is redeemed; `setupCodeHash` + `setupCodeExpiresAt` hold the pending
 *   invite, and `setupCodeRedeemedAt` stamps the single redemption.
 */
export type User = EntityEnvelope & {
  ...
  /** When the pending setup code expires (UTC, Clock port); `null` when no code is pending. */
...
export function isSetupCodePending(user: User, now: Date): boolean {
  return (
    ...
    now.getTime() <= user.setupCodeExpiresAt.getTime()
  );
}
