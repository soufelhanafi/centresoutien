...
/**
 * Password strength for a redeemed setup code. Mirrors the owner's account
 * schema (min length + character-class variety) so every credential in the
 * center meets the same bar.
 */
 ...
 * role is bounded as a non-empty string here and validated against the known
 * {@link Role} set fail-closed in the use case (SOU-95) — a single home for the
 * role check that also guards a value deserialized past the type system.
 */
...
/**
 * Fields an invited employee supplies at first login: their username, the
 * one-time setup code, and the password they choose. The setup code is only
 * bounded (never re-checked against password strength — it is a system-minted
 * token, not a user secret).
 */
