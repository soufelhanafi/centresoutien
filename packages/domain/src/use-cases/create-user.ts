...
/**
 * A human-readable one-time setup code (e.g. `A7K2-9FMP-3QRT`). Crockford-ish
 * alphabet excluding easily confused glyphs (0/O, 1/I), drawn from the
 * cryptographically secure {@link SecureRandom} with rejection sampling so the
...
function generateSetupCode(random: SecureRandom): string {
  ...
  const chars: string[] = [];
  let pool = random.bytes(total * 2);
  let pos = 0;
  const ceiling = 256 - (256 % SETUP_CODE_ALPHABET.length);
  while (chars.length < total) {
    if (pos >= pool.length) {
      pool = random.bytes(total * 2);
      pos = 0;
    }
    const b = pool[pos++]!;
    if (b < ceiling) chars.push(SETUP_CODE_ALPHABET.charAt(b % SETUP_CODE_ALPHABET.length));
  }
  const groups: string[] = [];
  for (let g = 0; g < SETUP_CODE_GROUPS; g++) {
    groups.push(chars.slice(g * SETUP_CODE_GROUP_LEN, (g + 1) * SETUP_CODE_GROUP_LEN).join(''));
  }
  return groups.join('-');
}
