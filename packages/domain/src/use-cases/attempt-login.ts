 ...
 * session (SOU-256): the login screen no longer just learns "in", it learns
 * which user and role are now active.
 */
...
/**
 * Verifies a credential pair, returning the {@link AuthenticatedUser} on a match
 * or `null` otherwise. Implemented by `VerifyUserPassword`; stubbed in tests.
 */
 ...
 * carries the resolved {@link AuthenticatedUser} so the session knows who is in.
 */
 ...
 * wrong password — counts equally toward the shared console's lock.
 */
