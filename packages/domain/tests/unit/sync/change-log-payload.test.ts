...
describe('serializeChangeLogPayload', () => {
  it('wraps the entity in a versioned envelope with the current version', () => {
    const parsed = JSON.parse(serializeChangeLogPayload(SUBJECT)) as Record<string, unknown>;
...
describe('deserializeChangeLogPayload', () => {
  ...
  it('rejects a malformed envelope (missing version or entity)', () => {
    ...
    expect(() => deserializeChangeLogPayload('{"version":0,"entity":{}}')).toThrow(
      /malformed payload envelope/,
    );
    expect(() => deserializeChangeLogPayload('{"version":1,"entity":null}')).toThrow(
      /malformed payload envelope/,
    );
  });

  it('rejects a non-integer or unsafe version before slicing/upcasting', () => {
    expect(() => deserializeChangeLogPayload('{"version":1.5,"entity":{}}')).toThrow(
      /malformed payload envelope/,
    );
    expect(() => deserializeChangeLogPayload('{"version":-1,"entity":{}}')).toThrow(
      /malformed payload envelope/,
    );
    // 1e20 exceeds Number.MAX_SAFE_INTEGER — never a valid shape version.
    expect(() => deserializeChangeLogPayload('{"version":1e20,"entity":{}}')).toThrow(
      /malformed payload envelope/,
    );
  });

  it('rejects invalid JSON', () => {
    expect(() => deserializeChangeLogPayload('not json')).toThrow(/not valid JSON/);
  });

  it('refuses a payload from a version whose upcaster this device lacks', () => {
    const future = JSON.stringify({ version: 2, entity: {} });
    expect(() => deserializeChangeLogPayload(future)).toThrow(/upcasters this device lacks/);
  });
});
