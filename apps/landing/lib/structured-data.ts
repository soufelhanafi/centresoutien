...
// Canonical host. The apex domain redirects to www (Netlify primary-domain
// redirect), so every SEO signal (canonical, hreflang, og:url, sitemap, robots
// host, JSON-LD urls) must use the www host to match the URL Google actually
// resolves — otherwise the canonical points at a URL that immediately redirects.
