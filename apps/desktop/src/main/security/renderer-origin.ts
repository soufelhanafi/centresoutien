 ...
 *
 * The packaged `file:` variant pins the trust to the exact index entry (host +
 * path), not the `file:` scheme alone (SOU-242): a `file:` URL has no comparable
 * `origin`, so without the pin the navigation guard would let a compromised
 * renderer redirect to any other local file — or, since a `file:` URL can carry
 * an authority (Windows UNC `file://server/share/…`), to a remote share with the
 * same path on an attacker host. `indexUrl` is the pre-computed `file:` href of
 * the renderer entry (`pathToFileURL(indexHtml).href`), passed in from the
 * composition root so this module stays free of `node:*` and unit-testable.
 */
 ...
 * packaged build that means the same `file:` entry — matching host AND path (a
 * `file:` authority is meaningful on Windows UNC shares, so a foreign host must
 * never pass), with search and hash ignored (`loadFile` appends `?locale=…` and
 * the SPA routes on the hash). In dev it means an exact origin match, so a
 * different port or host is refused. Both sides are normalized through `URL` and
 * a drive-letter fold, so percent-encoding, separators, and Windows casing
 * compare identically across platforms; a foreign `file:` host or path (or any
 * non-`file:` scheme) is refused.
 */
