 ...
 * platform-specific `.node` binary and needs no native rebuild or per-OS
 * packaging (unlike the former `@node-rs/argon2` adapter). This adapter is the
 * only place the plaintext password is handled; the produced PHC string is all
 * that ever reaches the DB (SOU-26).
 *
