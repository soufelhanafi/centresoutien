...
import { RECOVERY_CODE_CONSUME_SQL } from './recovery-code-repository';
import { AUTH_AUDIT_LOG_INSERT_SQL, authAuditEventToParams } from './auth-audit-log-repository';
import { DEVICE_SESSION_CLEAR_SQL } from './device-session-store';

/**
 * SQLite adapter for {@link RecoveryCodeResetUnitOfWork} (SOU-169). Runs the
 * whole reset — password write, code consume, optional device-session clear, and
 * the audit rows — inside a SINGLE `db.transaction`, so a throw at any step rolls
 * the lot back and the code is never spent on an unchanged password. It owns no
 * SQL of its own: every statement is imported from the repository that owns that
 * table, so there is one source of truth per write. better-sqlite3 transactions
 * are synchronous and serialize on the single connection, so no concurrent writer
 * can observe a half-applied reset.
 */
export class SqliteRecoveryCodeResetUnitOfWork implements RecoveryCodeResetUnitOfWork {
  ...
  constructor(private readonly db: DB) {
    ...
    this.consumeCode = db.prepare(RECOVERY_CODE_CONSUME_SQL);
  ...
  async commit(unit: RecoveryCodeResetUnit): Promise<void> {
    const run = this.db.transaction((u: RecoveryCodeResetUnit): void => {
      ...
      this.consumeCode.run({ id: u.consumedCodeId, consumed_at: u.consumedAt.toISOString() });
      if (u.clearDeviceSession) this.clearSession.run();
