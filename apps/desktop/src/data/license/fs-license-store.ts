...
/**
 * Writes a validated license envelope to the machine-scoped license path
 * (SOU-315), the same path {@link Ed25519LicenseAdapter} reads at startup (both
 * derived from `licenseFileName`, so read + write always agree). The activation
 * use case validates before calling this; the store only persists.
 *
