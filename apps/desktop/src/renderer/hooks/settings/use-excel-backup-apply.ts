 ...
 * dry-run preview has been shown and the human confirms. The source is the same
 * main-issued dialog token the preview consumed.
 */
export function useExcelBackupApply() {
  return useMutation({
    mutationFn: (input: { pathToken: string }) => window.api.invoke('backup.excel.apply', input),
  });
}
