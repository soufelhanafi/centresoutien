 ...
 * the form reflects exactly what was persisted, and invalidates the schedule
 * audit so the stranded-session count reflects the new hours (SOU-251).
 */
export function useSaveCenterHours() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      queryClient.setQueryData(['center-hours'], data);
      void queryClient.invalidateQueries({ queryKey: scheduleAuditKeys.outsideHours() });
    },
  });
}
