...
/**
 * One now-out-of-window session in the post-save summary (SOU-283): its group,
...
export function TeacherAvailabilityRecheckRow({ session }: { session: OutOfWindowSessionView }) {
  ...
  const group = session.groupName
    ? localizedText(session.groupName, locale)
    : t('teachers.availability.recheck.unknownGroup');
