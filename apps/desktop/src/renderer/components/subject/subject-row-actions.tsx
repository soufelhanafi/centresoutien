 ...
 * when `canDelete` is false. The menu item is only *visually* muted in that case
 * (not the native `disabled` attribute, which would also block hover) so its
 * `title`/tooltip stays reachable and clicking still opens the informative modal
 * instead of doing nothing — friendlier for a low-tech-literacy user than a
