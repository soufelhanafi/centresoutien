 ...
 * `invalid-user-role`, `role-not-invitable`, plus the SOU-265 authorization
 * guards `insufficient-role` / `not-authenticated`) the shared Zod schema can
 * also reject with `role-required` / `username-too-short` / `username-too-long`
 * — those have their own translated messages, so surfacing the specific code
 * beats collapsing them into the generic form toast. `username-already-taken` is the one the caller
