...
describe('temp-pdf ownership', () => {
  it('recognizes only files under our own print prefixes', () => {
    expect(isOwnedTempPdfName('planning-week-abc.pdf')).toBe(true);
    expect(isOwnedTempPdfName('facture-inv_x-1.pdf')).toBe(true);
    expect(isOwnedTempPdfName('bulletin-paie-1.pdf')).toBe(true);
    expect(isOwnedTempPdfName('recu-paiement-1.pdf')).toBe(true);
...
describe('sweepStaleTempPdfsIn', () => {
  ...
  it('removes only stale owned PDFs and leaves fresh and foreign files alone', () => {
    const staleOwned = join(dir, 'planning-week-1.pdf');
    const freshOwned = join(dir, 'facture-inv_x-2.pdf');
...
describe('sweepStaleTempPdfs', () => {
  ...
  it('uses the default five-minute freshness threshold via the Electron temp dir', () => {
    const stale = join(dir, 'bulletin-paie-1.pdf');
    const fresh = join(dir, 'recu-paiement-2.pdf');
  ...
  it('honors a caller-supplied threshold and temp dir', () => {
    const oldFile = join(dir, 'facture-inv_x-1.pdf');
    const recentFile = join(dir, 'planning-week-2.pdf');
