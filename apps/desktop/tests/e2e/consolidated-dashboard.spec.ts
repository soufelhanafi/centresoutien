...
test('Scenario 1 — Essentiel: Consolidé tab shows the Premium upsell lock + CTA', async () => {
  ...
  await gotoConsolidatedTab(win, strings);
  await win.screenshot({ path: `test-results/consolidated-essentiel-${locale()}.png` });

  await expect(win.getByRole('tab', { name: strings.tabConsolidated })).toHaveAttribute('aria-selected', 'true');

  // Upsell lock present: multi-center title + unlock-with-Premium CTA.
  await expect(win.getByText(strings.lockTitle).first()).toBeVisible();
  await expect(win.getByRole('button', { name: strings.unlockCta }).first()).toBeVisible();

  // The Premium-only cloud link-out must be absent on a non-Premium plan.
  await expect(win.getByText(strings.cloudTitle)).toHaveCount(0);
...
test('Scenario 2 — Premium: Consolidé tab shows the web link-out, no upsell, no data', async () => {
  ...
  await expect(win.getByText(strings.cloudTitle).first()).toBeVisible();
  await expect(win.getByText(strings.cloudBody).first()).toBeVisible();

  // No upsell affordance on Premium: neither the lock title nor the unlock CTA.
  await expect(win.getByText(strings.lockTitle)).toHaveCount(0);
  await expect(win.getByRole('button', { name: strings.unlockCta })).toHaveCount(0);
  ...
  await expect(win.getByText(strings.kpiRevenue)).toHaveCount(0);
