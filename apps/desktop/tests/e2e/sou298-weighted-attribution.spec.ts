...
test('Scenario 6 — formula form enters per-subject prices with live sum-equals-total validation', async () => {
  ...
  // The tall per-subject-price dialog can push the footer button below the
  // headless window, and the shared DialogContent has no internal scroll, so even
  // a force click is refused ("outside of the viewport"). Submit via the button's
  // native click — viewport-independent and still exercises the real create path.
  await dialog
    .getByRole('button', { name: L.formula.create })
