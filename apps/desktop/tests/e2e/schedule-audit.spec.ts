 ...
 *
 * The materialized window is the next two Mondays strictly after the runtime
 * clock (both future, so the non-past audit still lists them) — derived in the
 * fixtures, never hardcoded. Occurrences are generated FIRST, then stranded by an
...
test('Scenario 1 — override strands an occurrence: "outside hours" row shows names, date, time (not ids)', async () => {
  ...
  await seedGeneratedSessions(win, SEED_FIRST_ONLY.from, SEED_FIRST_ONLY.to); // materializes only the first Monday
  await narrowMondayOverride(win, SEED_FIRST_ONLY.from, SEED_FIRST_ONLY.to); // Monday 09:00–14:00 → 15:00–17:00 stranded
...
// it carries BOTH reasons (multi-reason, SOU-296). Domain grouping collapses
// the outside-hours finding into ONE group spanning both Mondays (×2), while
// the holiday finding is a separate single-date row for the second Monday:
//   first Monday  → outside hours only            → collapsed "Hors horaires ×2"
//   second Monday → outside hours + holiday        → "Jour férié" row + both badges
// ---------------------------------------------------------------------------
test('Scenario 2 — a both-affected occurrence carries both the holiday and outside-hours reasons', async () => {
  const L = STR[locale()];
  const D = DATE[locale()];
  live = await launch(locale());
  const win = live.win;

  await seedGeneratedSessions(win, SEED_BOTH.from, SEED_BOTH.to); // both Mondays
  await narrowMondayOverride(win, SEED_BOTH.from, SEED_BOTH.to); // both Mondays now outside hours
  await addHoliday(win, MONDAYS.second); // the second Monday is now ALSO a holiday
  await gotoAudit(win, L);

  await expect(win.locator('html')).toHaveAttribute('dir', L.dir);
  await expect(win.getByRole('heading', { name: L.pageTitle })).toBeVisible();

  // Two root-cause groups (not two dated rows): outside-hours (collapsed ×2) + holiday.
  ...
  await expect(outsideGroup).toContainText(D.second);

  // A separate single-date "Jour férié" row exists for the second Monday.
  ...
  await expandGroup(outsideGroup, L, 2);
  const secondMondayEntry = outsideGroup.locator('ul > li', { hasText: D.second });
  await expect(secondMondayEntry).toHaveCount(1);
  await expect(secondMondayEntry.getByText(L.reasonOutsideHours)).toBeVisible();
  await expect(secondMondayEntry.getByText(L.reasonHoliday)).toBeVisible();

  expect(await pageCrashed(win)).toBe(false);
  await win.screenshot({ path: `test-results/sou201-s2-both-reasons-${locale()}.png` });
});
...
test('Scenario 3 — cancelling one stranded occurrence keeps the weekly template and the other date', async () => {
  ...
  const seeded = await seedGeneratedSessions(win, SEED_BOTH.from, SEED_BOTH.to); // both Mondays
  await narrowMondayOverride(win, SEED_BOTH.from, SEED_BOTH.to); // both stranded (outside hours)
  ...
  // `.last()` targets the inner date row — the outer group card also matches by text.
  await rowForDate(win, D.first).last().getByRole('button', { name: L.cancelRowBtn }).click();
...
test('Scenario 4 — no stranded sessions shows the good empty state, not an error', async () => {
  ...
  await seedGeneratedSessions(win, SEED_BOTH.from, SEED_BOTH.to);
  await gotoAudit(win, L);
