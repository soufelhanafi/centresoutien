...
describe('PdfLibInvoiceRenderer', () => {
  ...
  it('renders each derived status on a single page', async () => {
    for (const input of [unpaid, partiallyPaid, paid, cancelled]) {
  ...
  it('drives visible content from status — each state produces different bytes', async () => {
    const [u, pp, p, c] = await Promise.all(
      [unpaid, partiallyPaid, paid, cancelled].map((input) => renderer.render(input)),
    );
    const buffers = [u, pp, p, c].map((bytes) => Buffer.from(bytes));
